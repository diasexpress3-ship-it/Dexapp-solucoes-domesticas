...
export default function ClienteDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Calculate stats (in a real app, this might be a separate collection or cloud function)
    ...
    <AppLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-black text-primary">Olá, {user?.nome.split(' ')[0]}!</h1>
            <p className="text-gray-500">Bem-vindo ao seu painel de controle.</p>
          </div>
          <Link to="/cliente/nova-solicitacao">
            <Button leftIcon={<Plus size={20} />} variant="secondary">Nova Solicitação</Button>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <Card className="bg-primary text-white border-none">
            <CardContent className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-bold text-gray-300 uppercase tracking-wider">Total Investido</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-primary">Solicitações Recentes</h2>
              <Link to="/cliente/agenda" className="text-sm font-bold text-accent hover:underline">Ver todas</Link>
            </div>

            {recentSolicitacoes.length > 0 ? (
              <div className="space-y-4">
                {recentSolicitacoes.map((sol) => (
                  <motion.div key={sol.id} whileHover={{ x: 5 }}>
                      ...
                      <CardContent className="p-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-6">
                          <div className="text-right hidden sm:block">
                            <p className="text-sm font-bold text-primary">{formatCurrency(sol.valorTotal)}</p>
              ...
              <Card className="border-dashed border-2 bg-transparent">
                <CardContent className="py-12 text-center">
                  ...
                  <h3 className="font-bold text-gray-700">Nenhuma solicitação encontrada</h3>
                  <p className="text-sm text-gray-500 mb-6">Comece agora solicitando seu primeiro serviço.</p>
                  <Link to="/cliente/nova-solicitacao">
                    <Button variant="secondary">Solicitar Agora</Button>
                  </Link>
                </CardContent>
              </Card>
            )}
          ...
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-primary">Minha Carteira</h2>
            <Card className="bg-gradient-to-br from-accent to-orange-600 text-white border-none">
              <CardContent className="p-6">
                <div className="flex justify-between items-start mb-8">
                  <div>
                    <p className="text-xs font-bold opacity-80 uppercase tracking-wider">Saldo Disponível</p>
            ...
            <div className="space-y-3">
              <h3 className="font-bold text-primary">Suporte Rápido</h3>
              <Card className="bg-gray-50 border-none">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">+258 84 000 0000</p>
                  </div>
                </CardContent>
              </Card>
            </div>
